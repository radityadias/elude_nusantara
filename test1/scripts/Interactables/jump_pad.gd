...
func _ready() -> void:
	...
	if AudioManager == null:
		print("ERROR: AudioManager AutoLoad not found in JumpPad script!")
...
func _on_body_entered(body: Node2D) -> void:
	...
	if body.has_method("get_input_direction"): # Contoh cara cek apakah ini player
		...
		print("JumpPad activated!") # Pesan debugging
		...
		if AudioManager: # Pastikan AudioManager tidak null
			...
			print("ERROR: AudioManager not found when trying to play jump pad sound!")
