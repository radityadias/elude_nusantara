...
func _ready() -> void:
	...
	if AudioManager == null:
		push_error("ERROR: AudioManager AutoLoad not found in PressButton.gd!")
	...
	if interaction_area: # Pastikan node anak ada
		...
		push_error("ERROR: PressButton: Child InteractionArea node not found!")
...
func _on_body_entered(body: Node2D) -> void:
	if body.is_in_group("player") and not is_pressed and can_press:
		...
		if AudioManager:
			...
			push_error("ERROR: PressButton: AudioManager is NULL when trying to play button sound!")
